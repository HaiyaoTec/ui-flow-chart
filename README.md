...
screens/          每屏原图与缩略图
session.jsonl     逐步事件流：状态转移、日志、预算触顶、控制指令、接管区间
events.jsonl      人工接管期的控件事件（只记控件标识，不含任何输入值）
ai.jsonl          AI 决策录像：每步问了什么要点、答了什么，供本机回放
```

主进程日志另落在 `app.getPath('logs')/main.log`（Windows 在 `%APPDATA%/Flow Chart/logs`）。

## 出问题时怎么查

用户机器上的问题，作者拿不到任何数据，所以证据得先留在本地、再由用户一键带出来。

**导出诊断包**：设置 → 诊断与日志 → 导出诊断包。内容分两级，勾选项就在按钮上方：

- 基础级：版本与运行环境、会话记录、状态转移序列、预算触顶详情、图谱结构与计数、
  预览几何自检、更新器状态、主进程日志尾部。**不含目标站的任何内容**，可以直接贴到公开的问题单里。
- 复现级：另加目标站地址、界面标题与页面校验提示原文，以及 AI 决策录像。

密钥、登录态、原始截图、接管期的控件文案任何级别都不含；「包含了什么、不含什么」
两份清单会写进包里，也在界面上列出来。

**回放一轮探索**：拿到复现级的诊断包之后，

```bash
npm run replay -- 收到的诊断包.json
```

它会在临时数据目录里建好项目、把 AI 请求接到本地的回放服务上，然后拉起应用自动开始探索——
当时的每个决策按原顺序重新作用到真实站点。页面与当时对不上时，控制台会指出是哪一步、
差在哪里，而不是默默给出一个错位的动作。
